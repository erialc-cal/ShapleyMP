""" Created by : L. Gan, L. Zhang
    LOCO-MP modified by Claire He
...
def LOCOMP_random(X,Y,n_ratio,B,model, selected_features=[],alpha=0.1,bonf=True):
    ...
    [predictions,in_mp_obs,in_mp_feature]= predictMP(X,Y,X,n_ratio,B,model)
    ...
    results = [get_loco(i,j) for i in range(N) for j in range(M)]
...
def predictMP(X,Y,X1,n_ratio,B,model):
    N = len(X)
    M = len(X[0])
    N1 = len(X1)
    ...
    predictions=[]
    for b in range(B):        
        [x_mp, y_mp, idx_I, idx_F] = get_minipatch(X,Y,n_ratio)
        model.fit(x_mp,y_mp)
        predictions.append(model.predict(X1[:, idx_F]))
...
def LOCOMPReg(X,Y,n_ratio,m_ratio,B,model, selected_features=[],alpha=0.1,bonf=True):
    ...
    [predictions,in_mp_obs,in_mp_feature]= predictMP_og(X,Y,X,n_ratio,m_ratio,B,model)
    predictions_train = predictions
...
def predictMP_og(X,Y,X1, n_ratio,m_ratio,B,model):
    N = len(X)
    M = len(X[0])
    N1 = len(X1)
    in_mp_obs,in_mp_feature = np.zeros((B,N),dtype=bool),np.zeros((B,M),dtype=bool)
